										...
										<table id="simple-table" class="table table-striped table-bordered table-hover">
											...
											<tbody>
												<c:forEach items="${list}" var="temp"><!--.items-->
													<tr>
														<td>${temp.productNum}</td>
														<td>${temp.productName}</td>
														<td>${temp.cityName}</td>
														<td>${temp.departureTime}</td>
														<td>${temp.productPrice}</td>
														<td>${temp.productDesc}</td>
														<%--<c:if test="${temp.userType == 0}">
														<td>启用</td>
														</c:if>
														...
														<td>
															<div class="hidden-sm hidden-xs btn-group">
																...
																<a href="${ctx}/admin/order_add?id=${temp.id}&productName=${temp.productName}&price=${temp.productPrice}" class="btn btn-xs btn-primary" style="margin-right: 4px;"><i class="ace-icon fa fa-wrench bigger-120"></i>下单</a>
																...
																<a href="${ctx}/admin/product_update?id=${temp.id}" class="btn btn-xs btn-primary" style="margin-right: 4px;"><i class="ace-icon fa fa-wrench bigger-120"></i>修改</a>
																</security:authorize>
															</div>																				
														</td>
													</tr>
...
<![endif]-->
		...
		function udstatus(id,productStatus){
			bootbox.confirm("<span class='text-primary'>" + "你确认修改产品状态？" + "</span>", function(result){
				if(result) {
					//

					$.ajax({
					    url:'${ctx}/product/updateProductStatus/'+id+'/'+productStatus,
					    type:'POST', //GET
		...
		function dell(id){
			bootbox.confirm("<span class='text-primary'>" + "你确认删除该用户？" + "</span>", function(result) {
				if(result) {
					$.ajax({
					    url:'${ctx}/product/deleteProductByPrimaryKey/'+id,
					    type:'POST', //GET
					    contentType : 'application/json', 
